...
def get_piece(board, row, column):
    ...
    """
    "*** YOUR CODE HERE ***"
    return board[row][column] if column >= 0 and column < len(board[row]) else board[row]
...
def put_piece(board, max_rows, column, player):
    ...
    for i in range(max_rows):
        if get_piece(board, max_rows - i -1, column) == '-':
            new_row = replace_elem(get_piece(board, max_rows - i -1, -1), column, player)
            new_board =  replace_elem(board, max_rows - i -1, new_row)
            row = max_rows - i - 1
            break
...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    for j in range(max_cols):
        ...
        if get_piece(board, row, j )==player:
            for k in range(num_connect):
                if num_connect+j <= max_cols and get_piece(board, row, k+j)==player:
                    win.append(1)
                if(len(win) == num_connect):
                    return True
...
def check_win_column(board, max_rows, max_cols, num_connect, col, player):
    ...
    for i in range(max_rows):
        ...
        if get_piece(board, i, col )==player:
                for k in range(num_connect):
                    if num_connect+i <= max_rows and get_piece(board, i+k, col)==player:
                        win.append(1)
                    #print(win)
                    if(len(win) == num_connect):
                        return True
...
def check_win(board, max_rows, max_cols, num_connect, row, col, player):
    ...
    diagonal_win = check_win_diagonal(board, max_rows, max_cols, num_connect,
                                      row, col, player)
    "*** YOUR CODE HERE ***"
    return check_win_column(board, max_rows, max_cols, num_connect, col, player) or check_win_row(board, max_rows, max_cols, num_connect, row, player) or diagonal_win
